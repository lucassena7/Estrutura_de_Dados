...
Triangulo *tri_cria(float x1, float y1, float x2, float y2, float x3,
                    float y3) {
...
void tri_acessa(Triangulo *p, float *x1, float *y1, float *x2, float *y2,
                float *x3, float *y3) {
 ...
 */
void tri_atribui(Triangulo *p, float x1, float y1, float x2, float y2, float x3,
                 float y3) {
...
char *tri_getTriangulo(Triangulo *p) {
  ...
  int tam = sizeof(*p) + 30;
  pt = (char *)malloc(tam);
  sprintf(pt, "A: %s  B: %s  C: %s", pto_getPonto(p->A), pto_getPonto(p->B),
          pto_getPonto(p->C));
  return pt;
}
