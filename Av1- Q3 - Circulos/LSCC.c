...
void Retira_sublista(tLSCC *l1, tLSCC *l2) {
  if (l1->prim->prox == l1->prim || l2->prim->prox == l2->prim) {
    return; // Se alguma das listas está vazia, não há nada para retirar
  }
  ...
  while (p1 != l1->prim) {
    if (p1->id == p2->id) {
      ...
      while (p2Rast != l2->prim && pAtual != l1->prim &&
             pAtual->id == p2Rast->id) {
        pAnt = pAtual;
        pAtual = pAtual->prox;
        p2Rast = p2Rast->prox;
      ...
      if (p2Rast == l2->prim) {
        // A sequência de nós de l2 foi encontrada em l1, vamos retirá-la
        pAnt->prox = pAtual;
        p1 = pAtual;
      } else {
        // A sequência de nós não coincide, avançamos p1
        p1 = p1->prox;
      }
    } else {
      // Os nós não coincidem, avançamos p1
      p1 = p1->prox;
    }
  }
}
