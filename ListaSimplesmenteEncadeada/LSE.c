...
int busca(tLSE *pL, int id, tNo **pAnt) {
  ...
  if (isClassif) {
    for (p = pL->prim, pRast = NULL; p != NULL && p->id < id;
         pRast = p, p = p->prox)
      ;
  } else {
    for (p = pL->prim, pRast = NULL; p != NULL && p->id != id;
...
/* lstSE_isClassif
  ...
  respectivamente.
*/
int lstSE_isClassif(tLSE *pL) { return pL->isClassif; }
...
/* lstSE_comRepet
  ...
  respectivamente.
*/
int lstSE_comRepet(tLSE *pL) { return pL->comRepet; }
...
/* lstSE_isVazia
  ...
  respectivamente.
*/
int lstSE_isVazia(tLSE *pL) { return pL->qtdNos; }
